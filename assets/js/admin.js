/* ============================================================
   ONES — Admin CMS (client-side, lưu localStorage + xuất data.js)
   ============================================================ */
(function () {
  "use strict";

  var PW_KEY = "pl_admin_pw", DEFAULT_PW = "ones2026", STORE = "pl_cms";
  var STATUS = ["Đang mở bán", "Đang phân phối", "Sắp ra mắt", "Sản phẩm chiến lược 2025", "Thứ cấp"];
  ...
  function formHTML(tab, it) {
    ...
    if (tab === "projects") return '<div class="fgrid">' +
      ...
      field("Nhãn (badge)", "f_badge", it.badge, { ph: "VD: Đối tác phân phối chính thức" }) +
      ...
      field("Sản phẩm / số phòng", "f_beds", it.beds, { ph: "VD: 1–3 PN hoặc Căn hộ · Biệt thự" }) +
      field("Diện tích", "f_size", it.size, { ph: "VD: 50 – 121 m²" }) +
      field("Bàn giao", "f_handover", it.handover, { ph: "VD: 2027" }) +
      field("Quy mô", "f_scale", it.scale, { ph: "VD: 117 ha" }) +
      field("Số lượng sản phẩm", "f_units", it.units, { ph: "VD: 3.175 căn hộ" }) +
      field("Nổi bật trang chủ", "f_featured", it.featured ? "Có" : "Không", { type: "select", options: ["Không", "Có"], hint: "Tối đa 6 dự án nổi bật hiện ở trang chủ" }) +
      imgField("Ảnh bìa", "f_cover", it.cover) +
      imgField("Thẻ sản phẩm OneS (ảnh tải về)", "f_card", it.card) +
    ...
    if (tab === "partners") return '<div class="fgrid">' +
      ...
      field("Trạng thái", "f_status", PARTNER_STATUS_LABEL[it.status] || PARTNER_STATUS_LABEL.active, { type: "select", options: [PARTNER_STATUS_LABEL.active, PARTNER_STATUS_LABEL.inactive], hint: "Đặt \"Đã ngừng hợp tác\" ngay khi một người rời OneS — trang tra cứu sẽ cảnh báo khách hàng" }) +
  ...
  function collect(tab) {
    ...
    if (tab === "projects") {
      ...
      return {
        ...
        area: g("f_area"), segment: g("f_segment"), type: g("f_type"), status: st, featured: g("f_featured") === "Có",
        badge: g("f_badge"), priceText: g("f_priceText"), priceValue: parseFloat(g("f_priceValue")) || 0,
        beds: g("f_beds"), size: g("f_size"), scale: g("f_scale"), units: g("f_units"), handover: g("f_handover"), cover: g("f_cover"), card: g("f_card"),
  ...
  function settingsView() {
    ...
    return '<div class="admin-help">Thông tin dưới đây dùng chung cho toàn website (chân trang, liên hệ, nút Zalo…). 🔑 Đặc biệt: dán mã <b>Formspree</b> vào ô "Form endpoint" để nhận khách qua form.</div>' +
      ...
      '<div style="background:var(--white);border:1px solid var(--line-soft);border-radius:12px;padding:1.4rem 1.5rem;max-width:760px;margin-top:1.2rem">' +
        ...
        '<div style="display:flex;gap:1.2rem;flex-wrap:wrap;align-items:flex-start">' +
          ...
          '<div style="flex:1;min-width:240px">' +
            ...
            '<label style="display:flex;align-items:center;gap:.5rem;margin:.8rem 0;font-size:.9rem"><input type="checkbox" id="ogBrand" checked> Phủ logo + slogan OneS</label>' +
      ...
      '<div style="background:var(--white);border:1px solid var(--line-soft);border-radius:12px;padding:1.4rem 1.5rem;max-width:760px;margin-top:1.2rem">' +
        ...
        field("Upload preset (Unsigned)", "cl_preset", localStorage.getItem(CL_PRESET_KEY) || "", { ph: "VD: ones_unsigned" }) +
      ...
      '</div>' +
      '<div style="background:var(--white);border:1px solid var(--line-soft);border-radius:12px;padding:1.4rem 1.5rem;max-width:760px;margin-top:1.2rem">' +
        '<h3 style="font-family:var(--head);font-size:1.1rem;margin-bottom:.4rem">🚀 Đăng web tự động (GitHub → hosting tự deploy)</h3>' +
        '<p style="color:var(--muted);font-size:.84rem;margin-bottom:.8rem">Sau khi cấu hình, bấm <b>"🚀 Đăng lên web"</b> trong thanh bên → tự commit lên GitHub → hosting (Tinh Gọn / Netlify) tự deploy → website live trong <b>~1 phút</b>.</p>' +
        '<details style="margin-bottom:.9rem;font-size:.84rem">' +
          '<summary style="cursor:pointer;font-weight:600;color:var(--red);user-select:none">▶ Hướng dẫn thiết lập lần đầu (10 phút)</summary>' +
          '<ol style="margin:.6rem 0 0 1.2rem;line-height:2">' +
            '<li>Tạo tài khoản <a href="https://github.com" target="_blank" rel="noopener" style="color:var(--red)">github.com</a> → <b>New repository</b> → tên <b>ones-website</b> → Public → Create</li>' +
        ...
        '</details>' +
        '<div class="fgrid">' +
        field("GitHub Repo", "gh_repo", localStorage.getItem(GH_REPO_KEY) || "", { ph: "khaibaodai/ones-website", hint: "Format: tên-tài-khoản/tên-repo" }) +
  ...
  function bindOgUploader() {
    ...
    function drawOg() {
      ...
      ctx.clearRect(0, 0, W, H); ctx.fillStyle = "#214E7F"; ctx.fillRect(0, 0, W, H);
      var sc = Math.max(W / ogImg.width, H / ogImg.height), iw = ogImg.width * sc, ih = ogImg.height * sc;
      ctx.drawImage(ogImg, (W - iw) / 2, (H - ih) / 2, iw, ih);
      if (ogBrand.checked) {
        var g = ctx.createLinearGradient(0, H * 0.4, 0, H); g.addColorStop(0, "rgba(18,38,63,0)"); g.addColorStop(1, "rgba(18,38,63,0.9)");
        ctx.fillStyle = g; ctx.fillRect(0, 0, W, H);
        ctx.fillStyle = "#00A04C"; ctx.fillRect(0, H - 12, W, 12);
        ...
        ctx.fillStyle = "#ffffff"; ctx.font = "700 40px 'Albula Pro', 'Segoe UI', sans-serif"; ctx.fillText("Kiến tạo giá trị thịnh vượng", 62, H - 52);
      }
  ...
  function generateDataJs() {
    var s = state, NL = "\n";
    var out = "/* OneS — assets/js/data.js (xuất từ trang quản trị " + new Date().toLocaleString("vi-VN") + ") */" + NL + NL;
...
"})();" + NL;
  ...
  function publishToGitHub() {
    ...
    if (location.protocol === "file:") { toast("⚠️ Phải mở admin qua URL website thật (https://...) để dùng tính năng này, không mở trực tiếp từ máy tính."); return; }
    ...
    fetch(apiUrl + "?ref=" + branch, { headers: headers })
      ...
      .then(function (file) {
        ...
        var body = { message: "Update OneS " + new Date().toLocaleDateString("vi-VN"), content: b64, branch: branch };
  ...
  function backup() {
    ...
    download("ones-noi-dung-" + new Date().toISOString().slice(0, 10) + ".json",
